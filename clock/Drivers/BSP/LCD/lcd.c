...
void lcd_show_chinese(uint16_t x, uint16_t y, char num, uint8_t size, uint16_t color)
{
    ...
    switch (size)
    {
        ...
        case 24:
            pfont = (uint8_t *)chinese_2424[num];  /* 调用2424字体 */
            break;

        case 32:
            pfont = (uint8_t *)chinese_3232[num];  /* 调用3232字体 */
            break;
		case 44:
            pfont = (uint8_t *)chinese_4444[num];  /* 调用4848字体 */
            break;
...
//lcd显示图片
void lcd_show_image(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t xuantu)
{
    uint16_t height, width;
    uint16_t i, j;
	uint16_t *color;
	...
	{
		case 1:
			color = (uint16_t *)gImage_duoyun;
		break;
		case 2:
			color = (uint16_t *)gImage_feng;
		break;
		case 3:
			color = (uint16_t *)gImage_qing;
		break;
		case 4:
			color = (uint16_t *)gImage_xue;
		break;
		case 5:
			color = (uint16_t *)gImage_ying;
		break;
		case 6:
			color = (uint16_t *)gImage_yu;
		break;
		default:
			break;
	}
